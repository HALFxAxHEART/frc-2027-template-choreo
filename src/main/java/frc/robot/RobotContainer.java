...
public class RobotContainer {
  ...
  // Choreo autonomous: builds trajectory-following commands from .traj files.
  ...
  public RobotContainer() {
    ...
    // ---- 2. Autonomous chooser (Choreo) ----
    // AutoFactory ties Choreo trajectories to the drivetrain: it reads the pose,
    // can reset the pose at the start of a path, and calls drive.followTrajectory()
    // for every step of the path. "true" auto-mirrors paths for the red alliance.
    autoFactory =
        new AutoFactory(
            drive::getPose, drive::setPose, drive::followTrajectory, true, drive);

    autoChooser = new LoggedDashboardChooser<>("Auto Choices");
    autoChooser.addDefaultOption("None", Commands.none());
    // Example: run a trajectory named "ExampleTrajectory" that you create in the
    // Choreo GUI app and save into src/main/deploy/choreo/. Rename to match your file.
    autoChooser.addOption(
        "Example Choreo Trajectory", autoFactory.trajectoryCmd("ExampleTrajectory"));
